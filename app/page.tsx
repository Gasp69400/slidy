  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mx-auto mt-12 grid max-w-5xl grid-cols-1 gap-4 sm:grid-cols-3 sm:items-stretch sm:gap-3 lg:gap-5">
          {tiers.map((tier) => (
            <Card
              key={tier.planId}
              className={cn(
                'relative flex flex-col overflow-hidden rounded-[1.25rem] border bg-white/95 text-slate-900 shadow-lg backdrop-blur-sm transition duration-300 dark:bg-slate-900/82 dark:text-white',
                tier.highlighted
                  ? 'border-violet-400/55 shadow-[0_24px_55px_-28px_rgba(109,40,217,0.45)] ring-2 ring-violet-500/45 dark:border-violet-400/40 dark:ring-violet-400/35'
                  : 'border-slate-200/75 shadow-slate-200/55 hover:border-violet-200/70 hover:shadow-[0_20px_45px_-32px_rgba(109,40,217,0.18)] dark:border-white/12 dark:hover:border-violet-500/25',
              )}
            >
              {tier.highlighted ? (
                <div className="absolute -top-2.5 left-1/2 z-10 -translate-x-1/2 whitespace-nowrap rounded-full bg-gradient-to-r from-violet-600 to-fuchsia-600 px-2.5 py-0.5 text-[10px] font-semibold leading-tight text-white shadow-md shadow-violet-900/25">
                  {t('pricing.popular')}
                </div>
              ) : null}
              <CardHeader className="space-y-1.5 p-4 pb-2">
                ...
                <CardDescription className="line-clamp-3 min-h-[3rem] text-left text-xs leading-snug text-slate-600 dark:text-slate-400">
                  {tier.description}
                </CardDescription>
                <div className="mt-2 flex flex-wrap items-baseline gap-0.5">
                  <span className="text-2xl font-semibold tabular-nums tracking-tight text-slate-900 dark:text-white sm:text-[1.65rem]">
                    {tier.price}
                  </span>
              ...
              <CardContent className="flex flex-1 flex-col p-4 pt-2">
                ...
                <ul className="space-y-1.5 text-xs leading-snug text-slate-600 dark:text-slate-400">
                  {tier.features.map((f) => (
              ...
              <CardFooter className="mt-auto p-4 pt-2">
                <Button
                  size="sm"
                  className={cn(
                    'h-9 w-full rounded-full text-xs font-semibold',
                    tier.highlighted
                      ? 'border-0 bg-gradient-to-r from-violet-600 to-violet-700 text-white shadow-md shadow-violet-900/25 hover:from-violet-500 hover:to-violet-600'
                      : 'border-violet-200/80 bg-white/90 text-slate-800 hover:border-violet-400 hover:bg-violet-50 dark:border-white/18 dark:bg-transparent dark:text-white dark:hover:bg-white/10',
                  )}
